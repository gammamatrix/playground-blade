<div class="card my-1">
    ...
    <div class="card-header">
        ...
        <h1>{{ __($data->getAttributeValue($packageInfo->model_attribute())) }}</h1>
    </div>
    @endif
    @if ($withImage && $data && $data->image)
        <div class="card-header">
            <img class="card-img-top" src="{{ $data->image }}" alt="{{ __(':model_label Image', ['model_label' => $packageInfo->model_label()]) }}">
        </div>
    @endif
    @if($withCardBody)
    <div class="card-body">
