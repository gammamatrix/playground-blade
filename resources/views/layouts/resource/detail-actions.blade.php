            ...
            @if ($data->trashed())
                <form method="POST" action="{{ $routeRestore }}" novalidate>
                    ...
                    <button class="dropdown-item text-success" role="button">
                        {{ __("Restore") }}
                        <span class="fa-solid fa-recycle float-end"></span>
                    </button>
                </form>
            @else
                @if (! $data->locked)
            ...
            @if ($data->locked)
                <form method="POST" action="{{ $routeUnlock }}" novalidate>
                    ...
                    <button class="dropdown-item text-warning" role="button">
                        {{ __("Unlock") }}
                        <span class="fa-solid fa-lock-open float-end"></span>
                    </button>
                </form>
            @else
                @if (! $data->trashed())
                    <form method="POST" action="{{ $routeLock }}" novalidate>
                        ...
                        >
                            {{ __("Lock") }}
                            <i class="fa-solid fa-lock float-end"></i>
                        </button>
                    </form>
