 ...
 */
$appName =
    isset($appName) && is_string($appName) && ! empty($appName)
        ? $appName
        : config("app.name");
...
>
     ...
     */
    $withBodyClass =
        isset($withBodyClass) && is_string($withBodyClass)
            ? $withBodyClass
     ...
     */
    $withMainClass =
        isset($withMainClass) && is_string($withMainClass)
            ? $withMainClass
            : "flex-shrink-0";
     ...
     */
    $withFooter =
        isset($withFooter) && (is_bool($withFooter) || is_string($withFooter))
            ? $withFooter
            : false;
     ...
     */
    $withUserMenu =
        isset($withUserMenu) && is_bool($withUserMenu) ? $withUserMenu : true;

    /**
     * @var boolean|string $withNav Show the navigation in the layout.
     */
    $withNav =
        isset($withNav) && (is_bool($withNav) || is_string($withNav))
            ? $withNav
            : true;
     ...
     */
    $withSidebarLeft =
        isset($withSidebarLeft) &&
        (is_bool($withSidebarLeft) || is_string($withSidebarLeft))
            ? $withSidebarLeft
            : false;
     ...
     */
    $withSidebarRight =
        isset($withSidebarRight) &&
        (is_bool($withSidebarRight) || is_string($withSidebarRight))
            ? $withSidebarRight
            : false;
     ...
     */
    $withSnippets =
        isset($withSnippets) && is_bool($withSnippets) ? $withSnippets : true;
    ...
    $withPlayground =
        isset($withPlayground) && is_bool($withPlayground) ? $withPlayground : true;
    ...
    <head>
        ...
        <title>
            {{ ! empty($appName) ? sprintf('%1$s: ', $appName) : "" }}
            @yield("title")
        </title>
    ...
    <body class="{{ $withBodyClass }}" {!! $theme->bodyStyle() !!}>
        ...
        {{-- snippet-banner rank: {-3000, -2000} --}}

        @yield("pre-header")

        @stack("snippet-header")
        {{-- snippet-header rank: {-1999, -1000} --}}
        ...
        <main role="main" class="{{ $withMainClass }}">
            ...
            @yield("main")
            @stack("snippet-main")
            {{-- snippet-main rank === {0, 1000} --}}
            @yield("content")
            @stack("snippet-content")
            {{-- snippet-content rank === {1001, 2000}, {rank < -3000 || rank > 5000} --}}
            @yield("content-end")
            @stack("snippet-main-footer")
            {{-- snippet-main rank === {2001, 3000} --}}
