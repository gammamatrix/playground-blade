...
foreach ($snippets as $snippet) {
    ...
    if (! empty($snippet["meta"]) && is_array($snippet["meta"])) {
        if (
            isset($snippet["meta"]["container"]) &&
            is_array($snippet["meta"]["container"]) &&
        ...
        if (
            isset($snippet["meta"]["header"]) &&
            is_array($snippet["meta"]["header"]) &&
            isset($snippet["meta"]["header"]["level"]) &&
            is_numeric($snippet["meta"]["header"]["level"]) &&
            in_array((int) $snippet["meta"]["header"]["level"], [
                1,
                2,
                3,
                4,
    ...
    if (
        ! empty($snippet["snippet_type"]) &&
        is_string($snippet["snippet_type"]) &&
        in_array($snippet["snippet_type"], ["banner", "header", "footer"])
    ) {
        $withTitle = false;
    }
    ...
    $attributes = trim(
        implode(
            ...
            array_filter([
                sprintf(
                    'data-id="%1$s"',
                    ! empty($snippet["id"]) && is_string($snippet["id"])
                        ? $snippet["id"]
                        : "",
                ),
                sprintf(
                    'data-created_at="%1$s"',
                    ! empty($snippet["created_at"]) &&
                    is_string($snippet["created_at"])
                        ? $snippet["created_at"]
                        : "",
                ),
                sprintf(
                    'data-updated_at="%1$s"',
                    ! empty($snippet["updated_at"]) &&
                    is_string($snippet["updated_at"])
                        ? $snippet["updated_at"]
                        : "",
                ),
                sprintf(
                    'data-published_at="%1$s"',
                    ! empty($snippet["published_at"]) &&
                    is_string($snippet["published_at"])
                        ? $snippet["published_at"]
                        : "",
                ),
                sprintf(
                    'data-published="%1$s"',
                    ! empty($snippet["published"]) &&
                    is_string($snippet["published"])
                        ? $snippet["published"]
                        : "",
                ),
                sprintf(
                    'data-allow_public="%1$s"',
                    ! empty($snippet["allow_public"]) ? 1 : 0,
                ),
                sprintf(
                    'data-allow_guest="%1$s"',
                    ! empty($snippet["allow_guest"]) ? 1 : 0,
                ),
                sprintf(
                    'data-allow_admin="%1$s"',
                    ! empty($snippet["allow_admin"]) ? 1 : 0,
                ),
                sprintf(
                    'data-rank="%1$s"',
                    ! empty($snippet["rank"]) && is_string($snippet["rank"])
                        ? $snippet["rank"]
                        : "",
                ),
            ]),
...
@push($stack)
    @include(
        "playground::components/snippets/stack",
        [
