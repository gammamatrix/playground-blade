@foreach ($columns as $column => $columnMeta)
    ...
    $routeParameterKey =
        ! empty($routeParameterKey) && is_string($routeParameterKey)
            ? $routeParameterKey
            : "";
    $routeParameter =
        ! empty($routeParameter) && is_string($routeParameter)
            ? $routeParameter
            : "";
     ...
     */
    $accessor =
        isset($columnMeta["accessor"]) && is_string($columnMeta["accessor"])
            ? $columnMeta["accessor"]
            : "";
     ...
     */
    $property =
        isset($columnMeta["property"]) && is_string($columnMeta["property"])
            ? $columnMeta["property"]
            : "";

    $columnMeta["class"] =
        isset($columnMeta["class"]) && is_string($columnMeta["class"])
            ? $columnMeta["class"]
            : "";
    if (isset($columnMeta["classes"]) && is_array($columnMeta["classes"])) {
        $columnMeta["class"] .= empty($columnMeta["class"])
            ? $columnMeta["class"]
            : " " . $columnMeta["class"];
    }
    $columnMeta["linkType"] =
        isset($columnMeta["linkType"]) && is_string($columnMeta["linkType"])
            ? $columnMeta["linkType"]
            : "";
    $columnMeta["linkRoute"] =
        isset($columnMeta["linkRoute"]) && is_string($columnMeta["linkRoute"])
            ? $columnMeta["linkRoute"]
            : "";

    $columnMeta["type"] =
        isset($columnMeta["type"]) && is_string($columnMeta["type"])
            ? $columnMeta["type"]
            : "";
    $columnMeta["html"] =
        isset($columnMeta["html"]) &&
        is_bool($columnMeta["html"]) &&
        $columnMeta["html"];
    $columnMeta["action"] =
        isset($columnMeta["action"]) && is_string($columnMeta["action"])
            ? $columnMeta["action"]
            : "";

    $columnMeta["filter"] =
        isset($columnMeta["filter"]) &&
        is_string($columnMeta["filter"]) &&
        ! empty($record[$column])
            ? $columnMeta["filter"]
            : null;

    if ($columnMeta["filter"]) {
        $columnMeta["filter_id"] =
            empty($record[$columnMeta["filter"]]) ||
            ! is_string($record[$columnMeta["filter"]])
                ? ""
                : $record[$columnMeta["filter"]];
        $columnMeta["filter_css_id"] =
            "filter_" . $columnMeta["filter"] . "_" . $columnMeta["filter_id"];
        $columnMeta["filter_name"] = "filter[" . $columnMeta["filter"] . "][]";
        $columnMeta["filter_checked"] =
            ! empty($validated) &&
            is_array($validated) &&
            ! empty($validated["filter"]) &&
            is_array($validated["filter"]) &&
    ...
    $isFk = $isFk && ! empty($columnMeta["property"]);
    if ($preferLinkSlug) {
    ...
    if (empty($columnMeta["linkRoute"])) {
        ...
        $link = empty($record["slug"])
            ? ""
            : route($columnMeta["linkRoute"], [$routeParameter => $record["slug"]]);
    } elseif ($preferLinkId) {
        $link = empty($record[$routeParameterKey])
            ? ""
            : route($columnMeta["linkRoute"], [
                $routeParameter => $record[$routeParameterKey],
            ]);
    } elseif ($preferLinkGo) {
        $link = empty($record[$routeParameterKey])
            ? ""
            : route($columnMeta["linkRoute"], [
                "go" => $record[$routeParameterKey],
            ]);
    } elseif ($isFk) {
        if (
            ! empty($record[$column]) &&
            ! empty($columnMeta["routeParameter"]) &&
            is_string($columnMeta["routeParameter"]) &&
            ! empty($columnMeta["routeParameterKey"]) &&
            is_string($columnMeta["routeParameterKey"]) &&
            ! empty($record[$columnMeta["routeParameterKey"]]) &&
            is_string($record[$columnMeta["routeParameterKey"]])
        ) {
            $link = route($columnMeta["linkRoute"], [
                $columnMeta["routeParameter"] =>
                    $record[$columnMeta["routeParameterKey"]],
            ]);
        }
    }
    ...
    $isFlag =
        isset($columnMeta["flag"]) &&
        is_bool($columnMeta["flag"]) &&
        $columnMeta["flag"];
    $withImage =
        isset($columnMeta["with-image"]) &&
        is_bool($columnMeta["with-image"]) &&
        $columnMeta["with-image"];
    ...
    >
        @if ($hasLink)
            <a href="{{ $link }}">
                @include("playground::components/table/data-row-value")
            </a>
        @else
            @include("playground::components/table/data-row-value")
        @endif
        ...
        @if ($columnMeta["filter"])
            <div class="form-check">
                ...
                <a
                    href="{{ $filter_table_id . "filter[id][]=" . $columnMeta["filter_id"] }}"
