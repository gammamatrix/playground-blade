    ...
    <legend>
        <div class="float-end">
            ...
            <button class="btn btn-warning btn-toggler" type="button" data-bs-toggle="collapse"
                    data-bs-target="#{{$id}}-fieldset-filter-ids .container" aria-expanded="true"
                    ...
                    <fieldset class="mb-3">
                        ...
                        @foreach ($filter_ids_values as $id_index => $id)
                            ...
                            @if ('array' === $filter_ids_value_type)
                                ...
                                <a href="{{request()->fullUrlWithoutQuery(sprintf('filter.%s.%s', $column, $id_index))}}"
                            ...
                            @else
                                ...
                                <input type="hidden" name="filter[{{$column}}]" value="{{$id}}">
                                <a href="{{request()->fullUrlWithoutQuery(sprintf('filter.%s', $column))}}"
                                   class="btn btn-outline-warning">
                                    {{$id_label ?? $id}}
                                    <span class="fas fa-close"></span>
                                </a>
