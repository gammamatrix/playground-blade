<fieldset class="mb-3" id="{{ $id }}-fieldset-filter-ids">
    <legend>
        <div class="float-end">
            ...
            <button
                class="btn btn-warning btn-toggler"
                type="button"
                data-bs-toggle="collapse"
                data-bs-target="#{{ $id }}-fieldset-filter-ids .container"
                aria-expanded="true"
                aria-controls="{{ $id }}-fieldset-filter-ids .container"
            >
        ...
        <div class="row">
            @foreach ($meta["ids"] as $column => $meta_column)
                @php
                    ...
                    $type = ! empty($meta_column["type"]) && in_array($meta_column["type"], ["boolean", "string", "uuid"]) ? $meta_column["type"] : "";
                    ...
                    <fieldset class="mb-3">
                        ...
                        @foreach ($filter_ids_values as $id_index => $id)
                            ...
                            @php
                                $id_label = $paginator
                                    ->getCollection()
                                    ->filter(function ($item) use ($id, $column) {
                            ...
                            @if ("array" === $filter_ids_value_type)
                                ...
                                />
                                <a
                                    href="{{ request()->fullUrlWithoutQuery(sprintf("filter.%s.%s", $column, $id_index)) }}"
                            ...
                            @else
                                ...
                                />
                                <a
                                    href="{{ request()->fullUrlWithoutQuery(sprintf("filter.%s", $column)) }}"
                                    class="btn btn-outline-warning"
                                >
                                    {{ $id_label ?? $id }}
                                    <span class="fas fa-close"></span>
